...
EPOCHS = 100
h5 = 'CNN_weight_ResNet50_FC_01.h5' #使用する重みファイル名(存在しないファイル名を書くと学習を開始し，その名前で重みファイルを保存)
...
# コンパイル
model.compile(loss='categorical_crossentropy', optimizer=optimizers.SGD(lr=1e-3, momentum=0.9),metrics=['acc'])
###################EalyStopping#######################
"""
...
####################学習関連#########################
train_path='/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/train/'
val_path='/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/val/'
try:#学習結果がある場合読み込み
...
####################判別精度確認用####################
test_img_path='/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/test/'
...
for i, d in enumerate(img_dirs3):
    ...
    with open('file\\'+filename,mode='a') as f:
        ...
        f.write(d)
    for f4 in files4:
       test_img = np.array(load_img('/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/' + d + '/' + f4).resize((960, 600))) #960,600
       test_img_2 = cv2.imread('/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/' + d + '/' + f4,1)
       print(f4)
       #HSV化
...
#       cog=np.delete(data,0,0)
       ...
       for i in range(n):
           ...
           cv2.imwrite('/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/trim1/trim_test.jpg', trim)
...
#           cv2.rectangle(test_img_2, (int(ag_X_s), int(ag_Y_s)), (int(ag_X_f), int(ag_Y_f)), (0, 255, 0), 2)
           ...
           if (trim_w == ORG_trim_X) and (trim_h == ORG_trim_Y):
               ...
               test_img = np.array(load_img('/home/hachimine/Desktop/deeplearning_tutorial/fine_tune_test/dataset2/trim1/trim_test.jpg').resize((32, 32)))
               result = model.predict_classes(np.array([test_img / 255.]))
